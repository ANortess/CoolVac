...
public class MisiónSarah : MonoBehaviour
{
    ...
    public SangreBNegativo inicioDialogo;
    void Update()
    {
        inicioDialogo = FindObjectOfType<SangreBNegativo>();
        if (inicioDialogo.inicioSarah == true)
        {
    ...
    private void SiguienteLinea()
    {
        ...
        else
        {
            ...
            noRepetir = true;
            inicioDialogo.inicioSarah = false;
            sangreActiva = true;
    ...
    {
        if (collision.gameObject.CompareTag("Player") && inicioDialogo.inicioSarah == true)
        {
            JugadorEnRango = true;
            if (noRepetir == false)
            {
